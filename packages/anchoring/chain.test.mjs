...
import {
  HUB_GENESIS_HASH, V0_EXTENSIONS, configureV0Registry, createChainClient, encodeRuntimeValidation, inspectAnchorEvents,
...
test('only fixed-size nonempty history commitments may be signed', () => {
  ...
  assert.throws(() => validateAnchorPayload(empty), /Empty anchors/);
...
test('a finalized failed dispatch is never reported as an anchored commitment', () => {
  ...
  assert.deepEqual(inspectAnchorEvents(records, 2, prepared.signerAddress, prepared.payloadHex), { status: 'failed', dispatchError: 'BadOrigin' });
});
...
async function transportFixture() {
  ...
  const queryInfo = async (encoded) => { feeInputs.push(Buffer.from(encoded).toString('hex')); return { partialFee: codec('11386503') }; };
  const at = { registry, runtimeVersion: runtime, consts: { balances: { existentialDeposit: codec('100000000') } },
    query: {
      system: { account: async () => ({ nonce: codec(controls.nonce), data: { free: codec('200000000000'), frozen: codec('0'), reserved: codec('0') } }),
  ...
  const chain = await createChainClient({ signerAddress: pair.address }, { ApiClass, ProviderClass });
  const prepare = () => chain.prepare({ payloadHex: commitment(), seedHex: `0x${'01'.repeat(32)}` });
