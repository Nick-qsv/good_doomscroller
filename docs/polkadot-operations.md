...
**Status, September 14, 2026: live; the science expansion and its fourth anchor are finalized and independently verified; daily revision-3 schedule enabled.** The library now contains 265 passages across 14 books. The release added 60 science passages: 15 Darwin, 15 Faraday, 18 Somerville, and 12 Cavendish. Batch `dde3620e-9d3b-44ae-8626-840f5eeff6ca` anchored 144 current public receipts, including the 60 science additions and the unbatched current backlog, in block 20,619,467. [Inspect extrinsic 20,619,467-2](https://assethub-polkadot.subscan.io/extrinsic/20619467-2). Its exact 81,806-byte readable-rationale envelope, database membership proofs, task logs, all 144 public receipt histories, and public-chain result were cross-checked; the paid fee was 0.4097286503 DOT, below the one-time 0.5 DOT approval ceiling.

Task revision 4 and immutable image tag `science-rationales-20260913-v1` were approved only for the completed bounded one-time run. A staged disabled-first rollback restored the 03:00 UTC schedule and its exact IAM allowlist to revision 3, image `rationales-20260913-3`. Fresh AWS readback confirmed the schedule `ENABLED`, no running or pending task, and both worker alarms present and `OK`; their action arrays are empty, so no email or push recipient is configured. Terraform desired values match this live anchoring state. A full account-checked plan showed no anchoring resource action and was not applied because it still includes the known unrelated web EC2 replacement. The final readback is preserved in `infra/releases/science-rationales-2026-09-13/rollback-to-rev3-final-verification.json`.

The first batch committed 236 receipts. The user explicitly approved the ongoing cost increase and proceeding on mainnet. Terraform deployed the original 22 custody/audit resources and 26 additional worker resources. CloudTrail is logging with successful S3 and CloudWatch delivery, KMS encryption-key rotation is enabled, and the custody filters and alarms exist. The stored wallet key was recovered through `asm-exec`, its public address matched, and its local signing check passed. Re-running initialization preserved the existing wallet.

The dedicated signer lives in AWS account `119033256269`, profile `w0m`, region `us-east-2`. Terraform manages custody/audit resources in `infra/terraform/polkadot.tf` and the isolated worker in `infra/terraform/polkadot-worker.tf`. Wallet generation and all secret values stay outside Terraform state. Funding address: `12wmbcz2PqfsLdJHhpn12bbkR1Az1ydkEsoJDxkSjCm8Ue59`, native DOT on Polkadot Hub / Asset Hub mainnet. Public metadata is in `infra/polkadot/funding.json`; this file contains no private key. A finalized public-chain read after the fourth anchor measured **19.0881471507 DOT** at **2026-09-14 02:07:36 UTC**, with account and next nonce both `4`. The earlier **19.6606781016 DOT** reading at 2026-09-13 00:13:02 UTC is historical, as is the original pre-funding `starting-balance.json` snapshot.
...
At first launch, AWS `GetSchedule` confirmed the schedule was **enabled once daily at 03:00 UTC**, targeting task revision **2** with image tag `mainnet-20260913-2`. The first batch, `a6c85d1e-a90a-4fcd-9122-437edeb39b65`, committed **236 receipts** in finalized block **20,580,568** at **2026-09-13 00:09:12 UTC**. Its transaction hash is `0xd373951d89cb0677bdde4e93d2c1118521d987f45749717c21c0975e9e13ca0b`, extrinsic index **2**. [Inspect the transaction](https://assethub-polkadot.subscan.io/extrinsic/20580568-2).

The downloaded proof passed the independent Dwellir-based online verifier with exit `0`, and the separate Python source-proof verification passed. [The public passage page displays its anchored history](https://goodoomscroller.com/passages/fe293a46-6ab7-50c1-99d2-566775985907/verification). An earlier second worker run exited `0` with status `idle`, sent no transaction, and left the account nonce at `1` at that time. The release build passed **59 anchoring tests** against PostgreSQL with no failures or skips; the dedicated CI job covers the suite.
...
The first commitment used a **92-byte legacy `GDSANCH1` envelope** carrying the batch UUID, receipt count, Merkle root and previous root. Current revision 3 also supports readable-rationale `GDSANCH2` envelopes: it selects the longest ordered receipt prefix that fits the 128 KiB envelope bound and ordinary fee ceiling, then retains any remainder. The first transaction paid **0.0011386503 DOT**, matching its estimate; the fourth, much larger science envelope paid **0.4097286503 DOT** under its separate one-time approval. Planning for 400 legacy-sized stamps at the first fee gives **0.45546012 DOT**, plus the worker's **1 DOT operating reserve**. The current **19.0881471507 DOT** balance exceeds this allowance. Ordinary controls permit at most **0.005 DOT estimated fee per attempt**, **three prepared attempts daily**, and a conservative **2 DOT rolling 365-day fee allowance**. Unresolved attempts consume their maximum permitted fee against that allowance. The worker flags balances below **2 DOT** for attention. Future fees can change; exceeding a limit pauses work rather than increasing the allowance automatically.
...
The deployed worker adds two metric/alarm pairs: an anchoring error or low balance, and no successful or idle run for approximately 26 hours. Final readback found both alarms present and `OK` with `ActionsEnabled=true`; every action array was empty, so no notification recipient is configured. Inspect ECS stopped-task reasons, worker logs and Scheduler when investigating missed runs.
